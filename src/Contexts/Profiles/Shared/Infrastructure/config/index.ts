...
const profilesConfig = convict({
  ...
  mongo: {
    url: {
      ...
      format: String,
      env: 'MONGO_URL',
      default: 'mongodb://localhost:27017/profiles-backend-dev'
    }
  },
  rabbitmq: {
    connectionSettings: {
...
profilesConfig.loadFile([
  __dirname + '/default.json',
  __dirname + '/' + profilesConfig.get('env') + '.json'
]);
