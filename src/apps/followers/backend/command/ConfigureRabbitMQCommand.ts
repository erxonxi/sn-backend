...
export class ConfigureRabbitMQCommand {
  static async run() {
    const connection = container.get<RabbitMqConnection>('Followers.Shared.RabbitMQConnection');
    const { name: exchange } = container.get<RabbitMQConfig>(
      'Followers.Shared.RabbitMQConfig'
    ).exchangeSettings;
    await connection.connect();

    const configurer = container.get<RabbitMQConfigurer>('Followers.Shared.RabbitMQConfigurer');
